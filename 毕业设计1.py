...
#输入的用户数据是现实意义上的数值，但在这个计算公式里应该将现实意义上的数值进行转化，也就是：多少现实数值等同于赋权求和公式里每个影响系数的100分
user_scores = []

score_input1 = input(f"请输入「静态易受骗系数」的评分（单位：分）: ").strip()#输入的数值是多少，进入公式里计算的数值就是多少
user_scores.append(int(score_input1))

score_input2 = input(f"请输入「设备安全状态」的评分（单位：分）: ").strip()
score_input2a = 100 - int(score_input2)#设备安全状态越高，代入公式里计算的数值应该越低
user_scores.append(int(score_input2a))

score_input3 = input(f"请输入「高风险网站平台访问频率」的次数（单位：次）: ").strip()#输入的数值是多少，进入公式里计算的数值就是多少
user_scores.append(int(score_input3))

score_input4 = input(f"请输入「高风险网站平台访问时长」的数值（单位：小时）: ").strip() #按照5小时来衡量100分
score_input4a = float(score_input4)*20
user_scores.append(float(score_input4a))

score_input5 = input(f"请输入「高风险网站平台交易频率」的次数（单位：次）: ").strip()#按照现实经验，在高风险网站平台交易与受诈骗有很大的关联性，以5次折合100分
score_input5a = int(score_input5)*20
user_scores.append(int(score_input5a))

score_input6 = input(f"请输入「高风险网站平台交易总金额」的数值（单位：元）: ").strip() #以10000元折合成100分
score_input6a = int(score_input6)/100
user_scores.append(int(score_input6a))
print("将输入的数据转化，转化后的数值如下：")
print(user_scores)
print("="*50)
print("输入完成，开始计算...")
...
print("【评估结果】")
print(f"计算出的风险指数 Y = {user_risk_index:.2f}")
